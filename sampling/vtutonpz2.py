...
parser.add_argument('--vtk_path', default="", type=str)
...
def extract_VTK(filename):
    ...
    print('save not compressed')    
    print("File saved at ", file_name)

first_file = True
files_cfd = []
for filename in sorted(glob.glob(os.path.join(vtk_path,'*.vtu'))):
    print(filename)
