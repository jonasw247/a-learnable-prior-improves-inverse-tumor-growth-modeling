# implemented by Sergey Litvinov
...
def cmaes(fun, x0, sigma, g_max, trace=False, workers=0, parameterRange=None):
    ...
    for gen in range(1, g_max + 1):
        ...
        likelihood = [x[1] for x in list]
        prior = [x[2] for x in list]
        lossDir = [x[3] for x in list]

        ys, x0, x1, xs, likelihood, prior, lossDir = zip(*sorted(zip(ys, x0, x1, xs, likelihood, prior, lossDir)))
        xmean = wsum(xs)
        ...
        if trace:
            Trace.append(
                (gen * lambd, ys[0], xs[0], sigma, C, ps, pc, Cmu, C1, xmean, likelihood, prior, lossDir))
