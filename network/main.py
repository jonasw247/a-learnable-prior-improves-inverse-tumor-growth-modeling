...
parser.add_argument('--thFlair', default=0.1, type=float)
parser.add_argument('--thT1', default=0.6, type=float)
...
if isOnlyAtlas:
    ...
    data_path= "/mnt/Drive3/jonas/synthetic_data/2023_3_5___21_55_34_Sampled30k/Dataset/npz_data"
...
if do_finetune:
    train_dataset = Dataset2(data_path, starttrain, endtrain,
                        mri_threshold_path, necro_threshold_path,
                         ...
                         outputmode=outputmode, isOnlyAtlas=isOnlyAtlas, thresholds = [thFlairFix, thT1Fix])
else:
    train_dataset = Dataset2(data_path, starttrain, endtrain,
                        mri_threshold_path, necro_threshold_path,
                         num_thresholds=num_thresholds, includesft=includesft,
...
# Setting up model
if is_new_save or do_finetune:
    savelogdir = './log/' + currenttime + purpose  # '/home/ivan/ib/learnmorph/log/torchimpl/' + currenttime + purpose
    if do_finetune:
        savelogdir = savelogdir + '_thFlair_' + str(thFlairFix) + '_thT1_' + str(thT1Fix)
    writer = SummaryWriter(log_dir = savelogdir)
    writerval = SummaryWriter(log_dir = savelogdir + '/val')
...
if is_new_save:
    ...
    train(num_epochs, model, train_dataset, train_generator, optimizer, device, lossfunction, writer,
            ...
            dropoutrate, batch_size, numoutputs, learning_rate, lr_scheduler_rate, starttrain, endtrain, startval,
            endval, version, schedulername, lossfunctionname, seed, is_debug, optimizername, weight_decay_sgd,
            modelfun_name, lr_patience, includesft, outputmode, args, summarystring, additionalsummary, do_finetune
              )
